...
```{r message = F, echo = F}
don <- read.table('C:/Users/marin/Dropbox/M1MAS/S2/projet_visualisation/VisuMarineClement/VisuMarineClement/VisuMarineClement/musee.csv', header = T, sep = ',', dec = '.')
...
```
  ...
  Le deuxième graphique en bas décrit l'évolution des visiteurs sur la période 2013-2018 pour une région. Donc en haut quand l'utilisateur sélectionne une région, le graphique se met à jour et montre une courbe qui relit chaque point correspondant à la fréquentation des musées pour chaque année.
```{r echo = F, message = F}
x <- don %>% group_by(region) %>% summarise(mean(total.2013),
                                                      sum(total.2014),
                                                      sum(total.2015),
                                                      sum(total.2016),
                                                      sum(total.2017),
